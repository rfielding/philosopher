...
- --- WHITEBOARD (raw sketch) --- : What the user typed/sketched
- --- PARSED DIAGRAMS (mermaid source) --- : The mermaid code generated from the sketch
  - [Diagram N: sequence] - sequence diagrams
  - [Diagram N: state] - state machine diagrams  
  - [Notes N] - text that wasn't recognized as diagrams
- --- CURRENT LISP SPEC --- : The existing BoundedLISP specification

Use the mermaid source to understand the diagrams the user is seeing. Reference them when answering questions about the protocol flow or states.`
...
func handleChat(w http.ResponseWriter, r *http.Request) {
	...
	// Include whiteboard and diagram context if present
	if req.Whiteboard != "" || len(req.Diagrams) > 0 || req.CurrentSpec != "" {
		var context strings.Builder
		context.WriteString(req.Message)
		
		if req.Whiteboard != "" {
			context.WriteString("\n\n--- WHITEBOARD (raw sketch) ---\n")
			context.WriteString(req.Whiteboard)
			context.WriteString("\n--- END WHITEBOARD ---")
		}
		
		if len(req.Diagrams) > 0 {
			context.WriteString("\n\n--- PARSED DIAGRAMS (mermaid source) ---\n")
			for i, d := range req.Diagrams {
				if d.Mermaid != "" {
					context.WriteString(fmt.Sprintf("\n[Diagram %d: %s]\n", i+1, d.Type))
					context.WriteString(d.Mermaid)
					context.WriteString("\n")
				} else if d.Text != "" {
					context.WriteString(fmt.Sprintf("\n[Notes %d]\n", i+1))
			...
			context.WriteString("--- END DIAGRAMS ---")
		}
		
		if req.CurrentSpec != "" {
			context.WriteString("\n\n--- CURRENT LISP SPEC ---\n")
			context.WriteString(req.CurrentSpec)
			context.WriteString("\n--- END SPEC ---")
		}
		
		userMessage = context.String()
	}
	
	// Add user message (store original for display, use augmented for LLM)
...
Click '✨ AI' for smart interpretation."></textarea>
    ...
    <script>
        ...
        async function sendMessage() {
            ...
            const provider = document.getElementById('provider').value;
            const whiteboard = document.getElementById('whiteboard').value.trim();
            
            // Parse whiteboard into diagrams for context
            const sections = parseWhiteboardSections(whiteboard);
            const diagrams = [];
            sections.forEach(section => {
                if (section.type === 'sequence') {
                    ...
                    diagrams.push({ type: 'sequence', mermaid: code });
                } else if (section.type === 'state') {
                    const code = 'stateDiagram-v2' + NL + section.lines.map(l => {
                        const withAction = l.match(/(\w+)\s*--\(([^)]+)\)-->\s*(\w+)/);
                        if (withAction) return '    ' + withAction[1] + ' --> ' + withAction[3] + ' : ' + withAction[2];
                    ...
                    diagrams.push({ type: 'state', mermaid: code });
                } else if (section.type === 'text' && section.lines.length > 0) {
                    diagrams.push({ type: 'notes', text: section.lines.join(NL) });
                }
            });
